...
export default function IssueHistory() {
  ...
  const filtered = history.filter(i => i.title.toLowerCase().includes(search.toLowerCase()) || i.id.toLowerCase().includes(search.toLowerCase()));

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <h1 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#1e293b' }}>Resolved Issue History</h1>
        <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Archive of completed municipal resolutions with before/after evidence.</p>
      </div>
      ...
      <div style={{ padding: '16px', background: '#fff', borderRadius: '12px', display: 'flex', alignItems: 'center', border: '1px solid #e2e8f0', width: '320px' }}>
        ...
        <input type="text" placeholder="Search history..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ border: 'none', outline: 'none', width: '100%', background: 'transparent' }} />
      </div>
          ...
          <div key={item.id} style={{ background: '#fff', borderRadius: '12px', padding: '20px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                ...
                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginTop: '4px' }}>{item.title}</h3>
              </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => { setSelectedIssue(item); setViewModalOpen(true); }} style={{ flex: 1, padding: '8px', fontSize: '0.8rem', background: '#f1f5f9', border: 'none', borderRadius: '6px', cursor: 'pointer' }}><FaEye /> View Details</button>
              <button onClick={() => alert(`Downloading report for ${item.id}`)} style={{ flex: 1, padding: '8px', fontSize: '0.8rem', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}><FaDownload /> Download</button>
            </div>
          </div>
        ))}
      </div>
